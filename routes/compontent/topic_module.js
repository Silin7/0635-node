...
const topic_list = (req, res, next) => {
  ...
  let slimit = (data.page - 1) * data.limit
  let elimit = data.limit
  let sql1 = `SELECT COUNT(*) FROM \`topic_list\` WHERE \`topic_class\` = '${data.topic_class}' AND \`is_pass\` = '02'`
  let sql2 = `SELECT id, topic_title, topic_img, create_time FROM \`topic_list\` WHERE \`topic_class\` = '${data.topic_class}' AND \`is_pass\` = '02' ORDER BY \`create_time\` DESC LIMIT ${slimit},${elimit}`
  conn().query(sql1, function (err1, result1) {
    if(err1){
      res.json({
        code: 500,
